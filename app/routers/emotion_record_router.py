...
from app.models.emotion_record_model import EmotionRecordInDb, EmotionRecord, AllEmotionReportsResponse
...
def get_all_emotion_report_for_logged_user(
    ...
    response = emotion_record_crud.get_emotion_records_by_user_id(db, [current_user.id], for_team=False, include_feedbacks=include_feedbacks)
    if response is None:
...
def get_emotion_report_for_logged_user_by_emotion_name(
    ...
    response = emotion_record_crud.get_emotion_records_by_user_id_and_emotion_id(db, current_user.id, emotion_id, include_feedbacks=include_feedbacks)
    if response is None:
